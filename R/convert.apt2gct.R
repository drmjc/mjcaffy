...
#' 
#' @return none. it writes out gct,res,cls files.
#' 
...
convert.apt2gct <- function(
	...
	if( all.genes ) {
		...
		if( create.res ) export.broad.res(rma, calls, description=desc, file=out.res.all.file, unlog2=FALSE)
	}
	...
	if( expressed.genes ) {
		...
		if( create.res ) export.broad.res(rma, calls, description=desc, file=out.res.expressed.file, unlog2=FALSE)
	}
